...
#MATCH EVENTS
def startNewMatch():
    global started, kickedOff, paused, resuming, isCelebrating, isEnding, time, blueScore, redScore, whoseBall, inReplay
    started = True
    kickedOff = False
    paused = False
    resuming = False
    ...
    c=False
    # Reset trạng thái kỹ năng của tất cả người chơi
    for player in physicsEngine.playerList:
        if player.isbot>0:
            c=True
        player.isSambaDance = False
        player.sambaDanceTimer = 0
        player.sambaDanceBall = None
        player.isFreeNutmeg = False
        player.nutmegTimer = 0
        player.nutmegTargetBall = None
        player.nutmegTargetPlayer = None
        player.nutmegFreezeTimer = 0  # Reset hiệu ứng đóng băng
        player.isMagicalTurn = False
        player.magicalTurnTimer = 0
        player.magicalTurnBall = None
        player.isPerfectShot = False
        player.perfectShotTimer = 0
        player.perfectShotTargetBall = None
        player.isOppositeDirection = False
        player.OppositeDirectionTimer = 0
        player.isZombieDribbling = False
        player.ZombieDribblingTimer = 0
        player.isCS = False
        player.CSTimer = 0
        player.CSball = None
        for skill in player.skillCooldowns:
            player.skillCooldowns[skill]["current"] = 0  # Reset cooldown của tất cả kỹ năng
        for ball in physicsEngine.ballList:
            ball.immune = False  
            ball.slow = False 
            ball.besteal = None  
    playerAcceleration = 0.035 if not c else 0.025
...
def toStartingPositions():
    ...
    physicsEngine.ballList[0].immune = False
...
def update( win, clock, previousKeys, currentKeys, stadiumWidth):
    global redScore
    global blueScore
    ...
    #PHYSICS UPDATE
    if menu.gameSection == 2 and started and not paused and not resuming and not inReplay:
        physicObjects = physicsEngine.update(previousKeys, currentKeys, stadiumWidth)  # Truyền stadiumWidth
        if replaysTurnedOn:
...
def loadRecord(path):    
    ...
    record  = pickle.loads( zlib.decompress(compressed)) 
    return record
