...
def timeToCollision_circle_circle(obj1, obj2):
    #circle, circle
    #d = a*x^2 + b*x + c
    if obj1.nutmegFreezeTimer > 0 or obj2.nutmegFreezeTimer > 0:
        return 2  # Bỏ qua va chạm nếu bị đóng băng
...
def timeToCollision_ball_player(obj1, obj2):
    # Khóa bóng khi Nagi trong trạng thái Shoot Delay
    for player in playerList:
        if player.isShootDelay and player.avatar == "./assets/images/Nagi.png" and player.shootTargetBall == obj1:
            return 2
        if player.isFreeNutmeg and player.avatar == "./assets/images/Bachira.png" and player.nutmegTargetBall == obj1 and player.nutmegTimer > 156:
            return 2
        if player.isSambaDance and player.avatar == "./assets/images/Bachira.png" and player.sambaDanceBall == obj1:
            return 2
        if player.isDribblingLord and player.avatar == "./assets/images/Barou.png" and player.dribblingLordBall == obj1:
            return 2  # Khóa bóng khi Dribbling Lord
        if obj2.isBodyBlock and obj2.avatar == "./assets/images/Kunigami.png":
            return 2  # Không khóa bóng khi đang Body Block
        if player.isKingShot and player.avatar == "./assets/images/Barou.png" and player.kingShotTargetBall == obj1 and player.kingShotTimer > 0:
            return 2  # Khóa bóng khi KingShot lấy đà
    
    if obj2.isDashing and obj2.avatar == "./assets/images/Nagi.png":
        distance = ((obj1.x - obj2.x) ** 2 + (obj1.y - obj2.y) ** 2) ** 0.5
        if distance < (obj1.r + obj2.r + 10):
            return 0  # Va chạm nhẹ khi Dash
        return 2

    if obj2.kicking:
        d = (obj1.r + 1.12 * obj2.r)**2
    else:
        d = (obj1.r + obj2.r)**2

    c = (obj1.x - obj2.x)**2 + (obj1.y - obj2.y)**2
    if obj2.kicking and c < d:
        return 0
...
def collide_ball_player(obj1, obj2):
    ...
    kick_force = gameHandler.kickForce
    if obj2.isFlowState and obj2.avatar == "./assets/images/Isagi.png":
        kick_force *= obj2.flowStateKickBoost  # Tăng lực sút nếu trong Flow State
    ...
    if obj2.kicking:
        ...
        velRelativ = v1 - v2
        elasticity = 0.01 if (obj2.isDashing and obj2.avatar == "./assets/images/Nagi.png") else 1.2
        velRelativ *= obj1.mass * obj2.mass * elasticity / (obj1.mass + obj2.mass)
...
def update(previousKeys, currentKeys, stadiumWidth):
    for player in playerList:
        # player.steer(previousKeys, currentKeys, stadiumWidth) 
        if not player.dead:
            if player.isbot > 0:
                for ball in ballList:
                    player.botmove(player.isbot, ball)
            else:
                player.steer(previousKeys, currentKeys, stadiumWidth)
        else:
            print(f"{player.nick}: {player.nutmegFreezeTimer}")
    for ball in ballList:
        ball.upd()

    time = 1
    max_iterations = 100  # Giới hạn số lần lặp để tránh vô hạn
    iteration_count = 0

    while time > 0 and iteration_count < max_iterations:
        collisionMinTime = 2
        collisionType = ""
        collidedObj1 = None
        collidedObj2 = None
        ...
        for ball in ballList:
            ...
            for player in playerList:
                ...
                if thisTime < collisionMinTime:
                    ...
                    collidedObj1 = ball
                    collidedObj2 = player
        # Kiểm tra và đặt lại vị trí bóng nếu ra khỏi sân
        stadium_width = stadiumWidth  # Đã được truyền từ gameHandler
        stadium_height = gameHandler.stadiumHeight  # Lấy từ gameHandler
        for ball in ballList:
            # Biên sân: -stadiumWidth/2 đến stadiumWidth/2 (x), -stadiumHeight/2 đến stadiumHeight/2 (y)
            if (ball.x < -stadium_width / 2 or ball.x > stadium_width / 2 or
                ball.y < -stadium_height / 2 or ball.y > stadium_height / 2):
                print(f"Ball out of bounds at ({ball.x}, {ball.y}), respawning at center!")
                ball.x = 0  # Đặt lại ở giữa sân
                ball.y = 0
                ball.vx = 0  # Dừng vận tốc để tránh bay tiếp
                ball.vy = 0
    ...
    if iteration_count >= max_iterations:
        print("Warning: Max collision iterations reached, possible infinite loop avoided")
    for player in playerList:
        if not player.dead:
            if player.isbot > 0:
                for ball in ballList:
    ...
    for player in playerList:
        player.vx *= 0.95  # Thay từ 0.98 thành 0.95
        player.vy *= 0.95
